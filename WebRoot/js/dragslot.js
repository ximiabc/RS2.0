;(function($, window, document, undefined){
	...
	Dragslot.prototype = {
		...
		_dragStart : function(e){
			...
			this.placeholder.css('height', dragItem.outerHeight());
...
//			dragItem.css('width',dragItem.width() + 'px');
		...
		_dragMove : function(e){
			...
			if (this.pointEl.closest('.' + this.options.slotHandlerClass).length || this.pointEl.closest('.' + this.options.slotItemClass).length) {
                this.pointEl = this.pointEl.closest('.' + this.options.slotItemClass);
                 var before = e.pageY < (this.pointEl.offset().top + this.pointEl.outerHeight() / 2);
		...
		_dragEnd : function(e){
            ...
            self.pointEl = null;
            //鼠标点击后不动直接松开会报错，原因是self.toSlot未定义。暂时在 判断语句中加入self.toSlot&&进行处理
            if (self.toSlot&&self.toSlot.hasClass(self.options.emptySlotClass)) {
